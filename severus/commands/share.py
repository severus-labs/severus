...
def share(ctx: click.Context, name: str):
    ...
    for attempt in range(max_attempts):
        candidate_code = sharing.generate_share_code()
        if sharing.check_code_availability(candidate_code):
            share_code = candidate_code
            break
        click.echo(f"Code collision, retrying... ({attempt + 1}/{max_attempts})")
    
    if not share_code:
        click.echo("Error: Could not generate unique share code. Please try again.")
        return
    
    # Encrypt data for sharing
    try:
        encrypted_data = sharing.encrypt_for_sharing(share_data, share_code)
    except Exception as e:
        click.echo(f"Error: Could not encrypt data: {e}")
        return
    
    # Upload to server
    if sharing.upload_shared_data(share_code, encrypted_data):
        click.echo(f"✓ Share code: {share_code}")
        click.echo("Code expires in 10 minutes.")
    else:
