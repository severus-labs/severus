...
@click.pass_context
def receive(ctx: click.Context, share_id: str):
    ...
    private_key = click.prompt("Private key", hide_input=True)

    click.echo("Retrieving shared secret...")

    try:
        # Decode private key
        key_bytes = base64.urlsafe_b64decode(private_key)
    except Exception:
        click.echo("Error: Invalid private key format.")
        return

    # Download encrypted data using ID
    try:
        encrypted_data = sharing.download_shared_data(share_id)
    ...
    try:
        share_data = sharing.decrypt_from_sharing(encrypted_data, key_bytes)
    except Exception:
        click.echo("Error: Could not decrypt shared data. Invalid private key?")
        return
